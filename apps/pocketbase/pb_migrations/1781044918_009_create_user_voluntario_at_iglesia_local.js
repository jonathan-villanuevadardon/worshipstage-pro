...
migrate((app) => {
  ...
  record.set("email", "voluntario@iglesia.local");
  record.setPassword($os.getenv("INITIAL_VOLUNTEER_PASSWORD"));
  record.set("first_name", "Voluntario");
  record.set("last_name", "Iglesia");
  record.set("role", "volunteer");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "voluntario@iglesia.local");
    app.delete(record);
  } catch (e) {
