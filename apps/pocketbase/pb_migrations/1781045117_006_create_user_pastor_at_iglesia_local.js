...
migrate((app) => {
  ...
  record.set("email", "pastor@iglesia.local");
  record.setPassword($os.getenv("INITIAL_PASTOR_PASSWORD"));
  record.set("first_name", "Pastor");
  record.set("last_name", "Principal");
  record.set("role", "church_admin");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "pastor@iglesia.local");
    app.delete(record);
  } catch (e) {
