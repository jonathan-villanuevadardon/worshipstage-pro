...
migrate((app) => {
  ...
  record.set("email", "lider@iglesia.local");
  record.setPassword($os.getenv("INITIAL_LEADER_PASSWORD"));
  record.set("first_name", "L\u00edder");
  record.set("last_name", "de Alabanza");
  record.set("role", "worship_leader");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "lider@iglesia.local");
    app.delete(record);
  } catch (e) {
